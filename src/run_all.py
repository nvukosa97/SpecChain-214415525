...
subprocess.run(["python", ROOT / "src" / "00_validate_repo.py"], check=True)

subprocess.run(["python", ROOT / "src" / "01_collect_or_import.py"], check=True)

#Cleans review data, produces data/reviews_clean.jsonl
subprocess.run(["python", ROOT / "src" / "02_clean.py"], check=True)
...
subprocess.run(["python", ROOT / "src" / "05_personas_auto.py"], check=True)

#Generate requirements spec using personas. Spec file spec/spec_auto.md
subprocess.run(["python", ROOT / "src" / "06_spec_generate.py"], check=True)

#Generate tests using spec. Output file tests/tests_auto.json
subprocess.run(["python", ROOT / "src" / "07_tests_generate.py"], check=True)
...
subprocess.run(["python", ROOT / "src" / "08_metrics.py"], check=True)
